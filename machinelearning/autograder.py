...
class Tracker(object):
    ...
    def begin_q(self, q):
        ...
        text = 'Partie {}'.format(q.replace('q', 'P'))
        print('\n' + text)
        print('=' * len(text))
    ...
    def end_q(self):
        ...
        print('\n### Partie {}: {}/{} ###'.format(
            self.current_question.replace('q', 'P'),
        ...
        if self.gs:
            ...
            with open(RESULTS, 'w') as f:
                d = { 
                    ...
                    'output': 'The score respect the constraints'
                }
                json.dump(d, f)
        else:
            print("""
The model follows the clients guideline. With a strong accuracy
""")
...
def parse_options(argv):
    parser = optparse.OptionParser(description='Run public tests on the code')
    parser.set_defaults(
        edx_output=False,
        gs_output=False,
        no_graphics=False,
    ...
    parser.add_option('--question', '-q',
                      dest='grade_question',
                      default=None,
                      help='Grade only one question (e.g. `-q p1`)')
    ...
    if options.grade_question and options.grade_question.startswith('P'):
        options.grade_question = options.grade_question.replace('P', 'q', 1)
...
def main():
    ...
    if options.grade_question:
        if options.grade_question not in questions:
            print("ERROR: q {} does not exist".format(options.grade_question))
            sys.exit(1)
        else:
